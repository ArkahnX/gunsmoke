...
export default function Button(props: {
	...
	return (
		<button
			...
			class={`${props.color === "dark" ? "bg-[#1C2A32] text-[#EFEFEF]" : props.color === "light" ? "bg-[#C9C8CE] text-[#1C2A32]" : props.color === "red" ? "bg-[#944040] text-[#EFEFEF]" : ""} ${props.design === "custom" ? "h-12 max-w-87.5 px-6.5" : "h-14 max-w-87.5 min-w-60"} relative flex flex-row items-center overflow-hidden rounded-sm text-xl font-bold whitespace-nowrap shadow-sm shadow-black/50 ${props.disabled ? " opacity-50 cursor-default" : "cursor-pointer outline-3 outline-transparent transition transition-discrete duration-250 hover:outline-white hover:duration-0"}`}>
